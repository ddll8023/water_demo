...
- **避免过度工程化**：单组件专用的样式逻辑不应提升为变量，保持在组件内部即可。
...
- **必须**优先使用通用变量。
- **复用性原则**：确需新变量时，必须先评估是否有跨组件复用需求，仅在确认复用价值后才在 `variables.scss` 中补充通用变量。
- **组件专用逻辑保持内部**：单一组件特有的样式逻辑、计算方法、动画效果等应保持在组件内部，避免不必要的抽象。
...
- **复用性验证**：新增变量前确认至少有 2 个不同组件的复用需求。
- 组件中优先使用通用变量，避免不必要的业务专用变量依赖。
- 确需创建的业务专用变量正确分类存放在 `business/` 对应模块中。
- **避免过度抽象**：单组件专用逻辑保持在组件内部，不盲目提升为变量。
