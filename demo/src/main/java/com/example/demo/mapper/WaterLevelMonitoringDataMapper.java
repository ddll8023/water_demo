...
import com.example.demo.pojo.dto.monitoring.WaterLevelMonitoringDataResponseDTO;
 ...
 * 
 * 设计说明：
 * - 严格参考FlowMonitoringDataMapper的实现规范
 * - 支持分页查询、统计分析、图表数据等功能
 * - 使用MyBatis-Plus的BaseMapper提供基础CRUD操作
 * - 自定义SQL查询支持复杂的业务需求
 */
@Mapper
public interface WaterLevelMonitoringDataMapper extends BaseMapper<WaterLevelMonitoringData> {
     ...
     * @return 查询结果列表
     */
    List<WaterLevelMonitoringDataResponseDTO> selectWaterLevelMonitoringDataPage(
     ...
     * @return 统计数据Map
     */
    Map<String, Object> selectWaterLevelStatistics(
            @Param("stationId") Long stationId, 
     ...
     * @return 图表数据列表
     */
    List<Map<String, Object>> selectWaterLevelChartData(
            @Param("stationId") Long stationId,
            @Param("startTime") LocalDateTime startTime,
            @Param("endTime") LocalDateTime endTime,
     ...
     * @return 汇总图表数据列表
     */
    List<Map<String, Object>> selectAllStationsWaterLevelChartData(
            @Param("startTime") LocalDateTime startTime,
            @Param("endTime") LocalDateTime endTime,
            @Param("interval") String interval);
     ...
     * @return 水位数据Map
     */
    Map<String, Object> selectWaterLevelByStationAndTimeRange(
            @Param("stationId") Long stationId,
            @Param("startTime") LocalDateTime startTime,
            @Param("endTime") LocalDateTime endTime);
}
